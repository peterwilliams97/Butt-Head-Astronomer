...
lstm_path = join(SUBMISSION_DIR2, 'submission_gr_fasttext3.csv')
nb_path = join(SUBMISSION_DIR, 'tfidf_nb.csv')
glove_path = join(SUBMISSION_DIR, 'spacy_lstm20s.ALL.LINEAR2.csv')
ensemble_path = join(SUBMISSION_DIR, 'ensemble_nb_fasttext3_glove.csv')
...
ensemble = lstm.copy()
ensemble[LABEL_COLS] = (nb[LABEL_COLS] + lstm[LABEL_COLS] + glove[LABEL_COLS]) / 3

print('lstm:    ', dim(lstm))
print('nb:      ', dim(nb))
print('glove:   ', dim(glove))
print('ensemble:', dim(ensemble))

for i in 0, 1:
    assert lstm.shape[i] == nb.shape[i]
    assert lstm.shape[i] == ensemble.shape[i]
    assert glove.shape[i] == nb.shape[i]
    assert glove.shape[i] == ensemble.shape[i]
